...
class GitProjectUploadService {
  ...
  /// Faz uma inspeção somente-leitura para descobrir se o ZIP realmente
  /// alterará algum arquivo em `.github/workflows`. Isso evita exigir o
  /// escopo clássico `workflow` quando o ZIP apenas contém uma cópia idêntica
  /// dos workflows que já estão na branch.
  Future<bool> workflowFilesWillChange({
    required ZipProjectPreview project,
    required String repositoryFullName,
    required String branch,
  }) async {
    if (!project.hasWorkflowFiles) return false;
    ...
    try {
      ...
      final refData = refResponse.data ?? const <String, dynamic>{};
      ...
      final commit = commitResponse.data ?? const <String, dynamic>{};
      ...
      final treeData = treeResponse.data ?? const <String, dynamic>{};
      if (treeData['truncated'] == true) {
        throw const UnexpectedAppException('BASE_TREE_TRUNCATED');
      }

      existingEntries = <String, _ExistingEntry>{};
      final rawTree = treeData['tree'];
      if (rawTree is List) {
      ...
      // Branch/repositório ainda sem árvore materializada: qualquer workflow
      // presente no ZIP será uma nova gravação.
      return true;
    }
    ...
    try {
      for (final entry in archive) {
        ...
        if (!isProtectedRepositoryInfrastructurePath(gitPath)) continue;
        ...
        final sha = GitObjectHash.blobSha(bytes);
